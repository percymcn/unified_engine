...
function LoginPageContent() {
  ...
  const handleGoogleLogin = () => {
    if (!isGoogleEnabled || !googleAuthUrl) {
            ...
            <>
              ...
              <div className="relative my-6">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="px-4 bg-gray-800/50 text-gray-500">
                    {isGoogleEnabled ? "or continue with" : ""}
                  </span>
                </div>
              </div>
              ...
              <div className="w-full">
                <button
                  ...
                  disabled={!isGoogleEnabled || isLoading || oauthLoading}
                  className="w-full flex items-center justify-center gap-2 py-3 bg-gray-900/50 border border-gray-600 rounded-lg text-gray-300 transition-all disabled:opacity-50 disabled:cursor-not-allowed relative group"
                  aria-label={isGoogleEnabled ? "Sign in with Google" : "Google OAuth not configured"}
                  title={!isGoogleEnabled ? "Admin must configure Google OAuth" : ""}
                >
                  {/* Google Icon */}
                  ...
                  <span className="text-sm font-medium">
                    {isGoogleEnabled ? "Google" : "Google (Not Configured)"}
                  </span>
                  {!isGoogleEnabled && (
                    <Info className="w-4 h-4 text-gray-500 ml-auto" />
                  )}
                </button>
                {!isGoogleEnabled && (
                  <p className="mt-2 text-xs text-gray-500 text-center">
                    Admin must configure Google OAuth
                  </p>
                )}
              </div>
            </>
