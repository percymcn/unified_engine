...
# ============================================================
# LUXALGO WEBHOOK ENDPOINT
# ============================================================
@router.post("/luxalgo/{webhook_key}")
async def luxalgo_webhook(
    webhook_key: str,
    request: Request,
    background_tasks: BackgroundTasks,
    db: Session = Depends(get_db)
    ...
    """
    Dedicated endpoint for LuxAlgo TradingView alerts.
    ...
    Expected payload:
    {
        ...
        "price": 5234.50,
        "timeframe": "5",
        "details": { "type": "choch+", "order_block": true }
    }
    """
    try:
        if not webhook_key or len(webhook_key) < 10:
        ...
        # Process through LuxAlgo bridge
        from app.services.luxalgo_integration import get_luxalgo_bridge
        bridge = get_luxalgo_bridge()
        smartflow_signal = bridge.process_webhook(payload)

        if not smartflow_signal:
            return {"status": "filtered", "message": "Signal below confidence threshold or duplicate"}

        # Inject into standard TradingView webhook pipeline
        # Merge with webhook_key so it routes through existing config
        smartflow_signal["webhook_key"] = webhook_key
        smartflow_signal["luxalgo"] = True

        # Use existing multi-account routing
        webhook_config = db.query(WebhookConfig).filter(
            WebhookConfig.webhook_key == webhook_key,
            WebhookConfig.is_active == True
        ).first()
        ...
        if webhook_config:
            from app.routers.webhook_execute import execute_tradingview_signal
            request._json = smartflow_signal
            request._body = json.dumps(smartflow_signal).encode()
            result = await execute_tradingview_signal(request, db)
            return {"status": "processed", "luxalgo_signal": smartflow_signal, "result": result}

        return {
            "status": "accepted",
            "luxalgo_signal": smartflow_signal,
            "message": "No WebhookConfig found for key — set up a webhook in TradeFlow dashboard"
        }
